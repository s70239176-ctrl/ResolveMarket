  ...
  return (
    <main className="mx-auto max-w-[1600px] px-5 py-10 md:px-10 md:py-16">
      ...
      <section className="grid gap-12 py-12 lg:grid-cols-[1.2fr_0.8fr] lg:py-20">
        <div><p className="mono text-xs text-muted">THE QUESTION</p><h1 className="display mt-6 max-w-5xl text-[clamp(3.5rem,7vw,8rem)] font-black uppercase leading-[0.86]">{market.question}</h1><p className="mt-8 max-w-2xl text-lg leading-8 text-muted">{market.description}</p><div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm font-bold uppercase tracking-[0.12em]"><a className="inline-flex items-center gap-2 hover:text-red" href={market.source_url} target="_blank" rel="noreferrer">Primary evidence <ArrowUpRight size={16} /></a><a className="inline-flex items-center gap-2 hover:text-red" href={market.backup_source_url} target="_blank" rel="noreferrer">Backup evidence <ArrowUpRight size={16} /></a></div></div>
      ...
      <section className="grid gap-10 border-t border-ink py-10 lg:grid-cols-[1fr_1fr]">
        ...
        <div className="border-t border-ink pt-5 lg:border-l lg:border-t-0 lg:pl-10"><p className="mono text-xs text-red">THE EVIDENCE</p><div className="mt-6 border-y border-ink"><div className="flex justify-between py-4"><span className="mono text-[10px] text-muted">PRIMARY + BACKUP</span><span className="mono text-[10px] text-muted">AVAILABLE</span></div><a className="block border-t border-black/20 py-5 text-xl font-black hover:text-red" href={market.source_url} target="_blank" rel="noreferrer">{market.source_name} / primary <ArrowUpRight className="inline" size={18} /></a><a className="block border-t border-black/20 py-5 text-xl font-black hover:text-red" href={market.backup_source_url} target="_blank" rel="noreferrer">Backup source <ArrowUpRight className="inline" size={18} /></a><div className="border-t border-black/20 py-5"><div className="mono text-[10px] text-muted">RESOLUTION METHOD</div><div className="mt-5 grid gap-3 text-sm font-bold"><span>PRIMARY SOURCE</span><span className="text-red">↓ BACKUP CONFIRMATION IF NEEDED</span><span className="text-red">↓ VALIDATOR CONSENSUS</span><span>↓ VERDICT</span></div></div></div><div className="mt-8"><div className="mono text-[10px] text-muted">DEADLINE</div><div className="mt-2 text-xl font-black">{closed ? "DEADLINE REACHED" : timeLeft(market.deadline)}</div></div></div>
      </section>
